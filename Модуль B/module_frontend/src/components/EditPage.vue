...
async function submit () {
  ...
  const response = await $fetch(`/posts/${route.params.id}`, 'post', new FormData(event.target))
...
<template>
  <div id="post-form-page" class="page">
    <div class="post-form">
      <h2 class="form-title">Редактирование публикации</h2>
      <form id="create-post-form" @submit.prevent="submit">
        <input type="hidden" name="_method" value="PATCH" />
        <div class="form-group">
          <label for="post-title">Заголовок</label>
          <input type="text" name="title" id="post-title" class="form-control" placeholder="Введите заголовок" :value="post?.title">
        </div>
        <div class="form-group">
        ...
        <div class="form-group text-right">
          ...
          <button type="submit" class="btn"><i class="fas fa-paper-plane"></i> Редактировать</button>
        </div>
      </form>
    </div>
  </div>
